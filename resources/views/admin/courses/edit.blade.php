                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="description" id="description">{{ old('description', $course->description) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('slug') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="slug" id="slug" value="{{ old('slug', $course->slug) }}">
                            @if($errors->has('slug'))
                        ...
                        <div class="form-group {{ $errors->has('start_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="start_date" id="start_date" value="{{ old('start_date', $course->start_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('published') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="published" id="published" value="1" {{ $course->published || old('published', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('teachers') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="teachers[]" id="teachers" multiple required>
                                @foreach($teachers as $id => $teacher)
                                    <option value="{{ $id }}" {{ (in_array($id, old('teachers', [])) || $course->teachers->contains($id)) ? 'selected' : '' }}>{{ $teacher }}</option>
                                @endforeach
                            </select>
